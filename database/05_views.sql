...
--
-- Views are owned by the table owner (postgres/superuser), so the view's
-- USING permissions bypass the underlying table grants. Users need only
-- SELECT on the view, not the base tables.
-- ============================================================================
...
CREATE OR REPLACE VIEW clinic.v_student_medical_history AS
SELECT
    c.consultation_id,
    c.check_in_time,
    c.chief_complaint,
...
CREATE OR REPLACE VIEW clinic.v_faculty_clearance AS
SELECT
    hc.clearance_id,
...
CREATE OR REPLACE VIEW clinic.v_nurse_dashboard AS
SELECT
    c.consultation_id,
    c.check_in_time,
    s.student_number,
...
CREATE OR REPLACE VIEW clinic.v_doctor_consultations AS
SELECT
    c.consultation_id,
    c.check_in_time,
    s.student_number,
...
CREATE OR REPLACE VIEW clinic.v_admin_user_overview AS
SELECT
    u.user_id,
    u.username,
...
CREATE OR REPLACE VIEW clinic.v_qr_checkin AS
SELECT
    q.qr_token,
    s.student_id,
    s.student_number,
