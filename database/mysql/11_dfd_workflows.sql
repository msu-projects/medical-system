...
BEGIN
    ...
    IF p_consultation_id IS NULL THEN
        IF p_student_number IS NULL OR p_attended_by IS NULL THEN
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'student_number and attended_by are required for a new consultation';
        END IF;
        ...
        VALUES (
            ...
            encrypt_data(COALESCE(p_diagnosis, '')),
            encrypt_data(p_treatment_notes),
            p_vitals_bp, p_vitals_temp, p_vitals_pulse, p_vitals_weight,
            COALESCE(p_status, 'ongoing')
        );

        SET v_consultation_id = LAST_INSERT_ID();
    ELSE
        IF NOT EXISTS (SELECT 1 FROM consultations WHERE consultation_id = p_consultation_id) THEN
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'Consultation does not exist';
        END IF;

        UPDATE consultations
        SET chief_complaint = COALESCE(p_chief_complaint, chief_complaint),
            diagnosis = IF(p_diagnosis IS NULL, diagnosis, encrypt_data(p_diagnosis)),
            treatment_notes = IF(p_treatment_notes IS NULL, treatment_notes, encrypt_data(p_treatment_notes)),
            vitals_bp = COALESCE(p_vitals_bp, vitals_bp),
            vitals_temp = COALESCE(p_vitals_temp, vitals_temp),
            vitals_pulse = COALESCE(p_vitals_pulse, vitals_pulse),
            vitals_weight = COALESCE(p_vitals_weight, vitals_weight),
            status = COALESCE(p_status, status)
        WHERE consultation_id = p_consultation_id;

        SET v_consultation_id = p_consultation_id;
    END IF;

    SELECT v_consultation_id AS consultation_id;
END //
...
BEGIN
    ...
    INSERT INTO prescriptions (consultation_id, prescribed_by, prescription_details, notes)
    VALUES (
        p_consultation_id,
        p_prescribed_by,
    ...
    SELECT v_prescription_id AS prescription_id;
END //
